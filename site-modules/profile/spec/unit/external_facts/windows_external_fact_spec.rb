...
describe 'windows_external_fact.ps1' do
  let(:facts_dir) { File.expand_path('../../../facts.d', __dir__) }
  let(:fact_file) { File.join(facts_dir, 'windows_external_fact.ps1') }
  ...
  it 'runs the script directly when the environment supports it' do
    script_output = skip_unless_external_fact_runnable!(
      fact_file,
      command: ['powershell', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', fact_file],
